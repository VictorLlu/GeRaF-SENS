...
sample_cfg = dict(
    ...
    sdf_offset = 0.00,                            # SDF iso-level offset for mesh/vis extraction
    ...
    freeze_inv_s_step = 0,                        # keep deviation-network inv_s frozen before this step
)

model=dict(
    type = "GeRaFStage2",
    with_grad_regression=0.1,
    with_depth_regression=1.0, 
    rt_backend="cuda",
    mf_backend="cuda",
    ...
    radar_cfg = radar,
    fix_power = False,
    tgt_sdf_ckpt_path="configs/neus/boxv1/neus_boxv1.py",
    init_cfg = dict(
        type='Pretrained',
        checkpoint='configs/geraf/bunnybv2/geraf2_bunnyboxv1_stage1.py',
    ),
    sdf_network = dict(
        type='SDFNetwork',
        d_out = 257,
    ...
    signal_network = dict(
        type="ReflectivePowerNetwork",
        cfg=dict(
            refelective_act = 'sigmoid',
            refelective_exp_max = None,
            light_power = 7.5644,
            reflective_init= 0.999,
        ),
    )
)
